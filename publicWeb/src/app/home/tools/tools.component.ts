...
export class ToolsComponent implements OnInit, OnDestroy {
  ...
  constructor(private _sanitizer:DomSanitizer,
              ...
              private db:ConexionesService,public router: Router, public route: ActivatedRoute) {
                ...
                this.db.getAllDataPopular("articulos").then((data)=>{
                  console.log(data)
                 
                  this.products= data;
                })
                this.db.getAllDataEstatus("estatus").then((data:any)=>{
                


                /*  for(var i=0;i<=data.length;i++){
                    this.productCollections.push(data[0].collection);
                  }*/

               

                let arreglo = data.filter((collection) => {
                
                    const index = this.productCollections.indexOf(collection['collection']);
                    if (index === -1) this.productCollections.push(collection['collection']);
                  })
